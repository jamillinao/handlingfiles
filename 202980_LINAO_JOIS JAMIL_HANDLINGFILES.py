    ...
    while True:
        ...
        if order[0] in products:
            if order[0] in orders:
                orders[order[0]]['item_quantity'] += int(order[1])
            else:
                orders[order[0]] = {'item_quantity': int(order[1])}
...
CODE\t\t\tNAME\t\t\tQUANTITY\t\t\tSUBTOTAL''')
        for item in codes:
            receipt.write(f"\n{item}\t\t{get_property(item, 'name')}\t\t{orders[item]['item_quantity']}\t\t\t\t{orders[item]['item_quantity'] * get_property(item, 'price')}")
            total += orders[item]['item_quantity'] * get_property(item, 'price')
        receipt.write(f'''\nTotal:\t\t\t\t\t\t\t\t\t\t{total}
==''')
